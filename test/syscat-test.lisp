...
(fiveam:test
  ...
  (let
    ...
    (fiveam:is (syscat::find-subnet *server* asn "" subnet1))
    ;; Remove the subnet
    (fiveam:is (syscat::delete-subnet *server* asn "" subnet1))
...
(fiveam:test
  ...
  (let
    ...
    ;; Confirm the subnet is there
    (fiveam:is (syscat::find-subnet *server* asn vrf subnet1))
    ;; Remove the subnet
    (fiveam:is (syscat::delete-subnet *server* asn vrf subnet1))
    ;; Confirm the subnet is gone
...
(fiveam:test
  ...
  "Create/read/delete tests on nested subnets directly under an ASN."
  (let
    ((asn "testco")
     (subnet1 "172.16.0.0/12")
...
(fiveam:test
  ...
  (let
    ...
    (fiveam:is (equal (list (first (cl-ppcre:split "/" subnet1)))
                      (syscat::find-subnet *server* asn "" subnet1)))
    ;; Add a second subnet
    (restagraph:log-message :debug "TEST Add a second-level subnet.")
    (fiveam:is (not (syscat::insert-subnet *server* asn "" subnet2)))
    ;; Confirm that's also there
    ...
    (fiveam:is (not (syscat::insert-subnet *server* asn "" subnet3)))
    ;; Confirm that's also there
    (restagraph:log-message :debug "TEST Confirm the new second-level subnet is present.")
    (fiveam:is (syscat::find-subnet *server* asn "" subnet3))
...
(fiveam:test
  ...
  (let ((asn "example")
    ...
    (fiveam:is (null (syscat::insert-ipv4address *server* address asn vrf)))
    (fiveam:is (equal (list (first (cl-ppcre:split "/" subnet)) address)
                      (syscat::find-ipv4address *server* address asn vrf)))
