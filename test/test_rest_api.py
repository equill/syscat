...
class TestIpv4SubnetsBasicNoVrf(unittest.TestCase):
    ...
    def test_create_and_delete_single_subnet(self):
        ...
        self.assertEqual(requests.post('%s/%s/organisations' % (BASE_URL, RAWPREFIX),
                                       data={'uid': ORGANISATION}).status_code,
                         201)
        # Add the subnet
        self.assertEqual(requests.post('%s/%s/subnets' % (BASE_URL, IPAMPREFIX),
                                       data={'org': ORGANISATION,
                                             'subnet': self.subnet1}).status_code,
                         201)
        # Confirm it's there
        self.assertEqual(requests.get('%s/%s/subnets?subnet=%s&org=%s'
                                      % (BASE_URL,
                                         IPAMPREFIX,
                                         self.subnet1,
                         ...
                         200)
        # Delete it
        self.assertEqual(requests.delete('%s/%s/subnets' % (BASE_URL, IPAMPREFIX),
                                         data={'org': ORGANISATION,
                                               'subnet': self.subnet1}).status_code,
                         204)
        # Ensure it's gone
        ...
        self.assertEqual(requests.delete('%s/%s/organisations/%s'
                                         % (BASE_URL, RAWPREFIX, ORGANISATION)).status_code,
                         204)
...
class TestIpv4AddressesBasicNoVrf(unittest.TestCase):
    ...
    def test_create_and_delete_single_address(self):
        ...
        self.assertEqual(requests.post('%s/%s/subnets' % (BASE_URL, IPAMPREFIX),
                         ...
                         201)
        # Add the address
        self.assertEqual(requests.post('%s/%s/addresses' % (BASE_URL, IPAMPREFIX),
                                       data={'org': ORGANISATION,
                                             'address': self.address1}).status_code,
                         201)
        # Confirm it's there
        self.assertEqual(requests.get('%s/%s/addresses?address=%s&org=%s'
                                      % (BASE_URL,
                                         IPAMPREFIX,
                                         self.address1,
                                         ORGANISATION)).status_code,
                         200)
        # Delete it
        self.assertEqual(requests.delete('%s/%s/addresses' % (BASE_URL, IPAMPREFIX),
                                         data={'org': ORGANISATION,
                                               'address': self.address1}).status_code,
                         204)
        # Ensure it's gone
        ...
        self.assertEqual(requests.delete('%s/%s/organisations/%s'
                                         % (BASE_URL, RAWPREFIX, ORGANISATION),
